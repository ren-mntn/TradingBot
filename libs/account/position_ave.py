...
# 建玉リスト管理クラス (部分クローズ時平均単価維持タイプ bybit/phemex インバースタイプ契約)
class OpenPositionKeepAve(object):
    ...
    def executed(self, id, side='', price=0, size=0, commission=0):
        ...
        if round(self._size+exec_qty,8)==0 :
            ...
            if self._order_currency=='USD' :
                profit = self._size/self.average_price-self._size/price-0.000000005
            else:
                profit = self._size*(price-self.average_price)/self.ref_ltp
    ...
    # インバースタイプの損益計算 (クリプト建て)
    @property
    def unreal(self):
        if self._order_currency=='USD' :
            return round(self._size/self.average_price-self._size/self.ref_ltp, 8) if self.ref_ltp!=0 and self._size!=0 else 0
        else:
            return round(self._size*(self.ref_ltp-self.average_price)/self.ref_ltp, 8) if self.ref_ltp!=0 and self._size!=0 else 0
