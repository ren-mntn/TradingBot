...
from libs.account import AccountInfo, OpenPositionKeepAve
...
# インバースタイプ契約
class BybitInverse(pybotters.BybitInverseDataStore, Bybit):
    ...
    def __init__(self, logger, candle_store, apikey=('',''), testnet=False):
        ...
        pybotters.BybitInverseDataStore.__init__(self)
    ...
    async def start(self, subscribe={}, symbol='BTCUSD'):
        ...
        # インバース契約
        if symbol[-3:]=="USD" :
            self._endpoint += "/realtime"

        # リニア契約
        else:
            self._endpoint += "/realtime_public"
        ...
        if self.auth:
            self._subscribe(topic="execution", handler=self._on_my_account)
            self._subscribe(topic="position", handler=self._on_my_positions)
            self._subscribe(topic="order", handler=self._on_my_orders)
            self._subscribe(topic="wallet", handler=None)

            # DataStoreの初期化
            await self.initialize(
                self._client.get('/v2/private/order', params={'symbol': self.symbol}),
                self._client.get('/v2/private/position/list', params={'symbol': self.symbol}),
                self._client.get('/v2/private/wallet/balance', params={'coin': self._collateral_coin[self.symbol]}),
            )

        # ローソク足受信処理タスクを開始
        asyncio.create_task(self._wait_candle_event(), name="bybit_inverse_candle")
        ...
        asyncio.create_task(self.ws._start_websocket(logging=False), name="bybit_inverse_websocket")

        # 自動キャンセルタスクを開始
        asyncio.create_task(self._cancel_task(), name="bybit_inverse_cancel")
    ...
    def _onmessage(self, msg, ws):
        self._logger.debug("recv bybit(inverse) websocket [{}]".format(msg.get('topic', 'None')))
        ...
        if topic_handler != None:
            ...
            if topic_handler in [self._on_ticker, self._on_my_account, self._on_my_positions, self._on_my_orders ] :
                pybotters.BybitInverseDataStore._onmessage(self,msg,ws)
    ...
    def _on_board(self,msg):
        ...
        if msg_type=='snapshot' :
            ...
            self.board_info.insert(recept_data,-1)
        elif msg_type=='delta' :
            self.board_info.delete(recept_data.get('delete',[]),-1)
            self.board_info.change(recept_data.get('update',[]),-1)
            self.board_info.insert(recept_data.get('insert',[]),-1)
    ...
    async def get_candles( self, timeframe, num_of_candle=500, symbol=None, since=None, need_result=True ):
        ...
        if target_symbol not in self._candle_subscribed:
            send_json = self._subscribe(topic=f"klineV2.1.{target_symbol}", handler=None)
        ...
        while keep_running and starttime<=endtime and num_of_candle!=0:
            ...
            try:
                responce = ''
                res = await self._client.get("/v2/public/kline/list", params={'symbol': target_symbol, 'interval': '1', 'from':starttime})
                ...
                # 次回の取得開始時
                starttime = int(rows[-1].get('start',rows[-1].get('open_time'))+api_timeframe)
                endtime = int(datetime.now(tz=timezone.utc).timestamp()//api_timeframe*api_timeframe)

                # 得られたデータが目的のendtimeを超えていたら終了
                if endtime < rows[-1].get('start',rows[-1].get('open_time')):
                    keep_running = False
    ...
    async def ticker_api(self, **kwrgs):
        try:
            res = await self._client.get('/v2/public/tickers', params={'symbol': kwrgs.get('symbol', self.symbol)})
            data = await res.json()
            self._update_api_limit(data)
            self.my.position.ref_ltp = data['result'][0]['last_price']
            return {'stat':data['ret_code'] , 'ltp':data['result'][0]['last_price'], 'msg':data}
    ...
    async def getpositions(self, **kwrgs):
        try:
            r = await self._client.get("/v2/private/position/list", params={'symbol':kwrgs.get('symbol', self.symbol)})
            res = await r.json()
    ...
    async def sendorder(self, order_type, side, size, price=0, auto_cancel_after=2592000, **kwargs ):
        ...
        params['qty']=int(self._round_order_size(size, symbol=params['symbol']))
        ...
        with self._order_dict_lock :
            try:
                r = await self._client.post('/v2/private/order/create', data=params)
                res = await r.json()
                self._update_api_limit(res)
            except Exception as e:
                self._logger.error(traceback.format_exc())
    ...
    async def cancelorder(self, id, **kwargs ):
        ...
        try:
            r = await self._client.post('/v2/private/order/cancel', data=kwargs)
            res = await r.json()
            self._update_api_limit(res)
        except Exception as e:
            self._logger.error(traceback.format_exc())
    ...
    async def changeorder(self, id, price, *kwargs ):
        try:
            ...
            r = await self._client.post('/v2/private/order/replace', data=params)
            res = await r.json()
            self._update_api_limit(res)
        except Exception as e:
            self._logger.error(traceback.format_exc())
